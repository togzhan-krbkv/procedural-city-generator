...
Live demo: https://togzhan-krbkv.github.io/procedural-city-generator/

## Status

All six milestones are complete: shape grammar floor plans, the street network with parcel division, block composition, extrusion to a JSON scene, a Three.js viewer, and deployment to GitHub Pages.
...
- A Three.js viewer, in TypeScript with Vite, that loads the exported scene and renders it with orbit camera controls
...
- The viewer remaps the generator's z-up coordinates to Three.js's y-up convention with a rotation, not a reflection, so it keeps every exported face winding outward without flipping any triangle. Buildings are flat shaded rather than smooth shaded so their corners read as sharp edges instead of a rounded blob. See `docs/algorithm-notes.md`.
- The scene fetch path is built from `import.meta.env.BASE_URL` instead of a hardcoded `/scenes/...`, since GitHub Pages serves this as a project site under `/procedural-city-generator/` rather than the domain root; a hardcoded path would 404 once deployed even though it works fine locally.
...
The same scene loaded and rendered in the Three.js viewer:

![City viewer](screenshots/milestone5-viewer.png)
